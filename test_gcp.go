...
func TestTerraformGcpExample(t *testing.T) {
	...
	exampleDir := test_structure.CopyTerraformFolderToTemp(t, "../", "terraform-gcp-example")
	...
	expectedBucketName := fmt.Sprintf("terratest_gcp", strings.ToLower(random.UniqueId()))

	// Also give the example instance a unique name
	expectedInstanceName := fmt.Sprintf("terratest_gcp", strings.ToLower(random.UniqueId()))
...
func TestSshAccessToComputeInstance(t *testing.T) {
	...
	sampleText := "systemctl status apache2"
	...
	retry.DoWithRetry(t, "Attempting to SSH", maxRetries, sleepBetweenRetries, func() (string, error) {
		output, err := ssh.CheckSshCommandE(t, host, fmt.Sprintf("sudo '%s'", sampleText))
		if err != nil {
			return "", err
		}

		if strings.Contains(activeText, output) {
